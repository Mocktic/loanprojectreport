...
def render_dataframe(df, na_rep=False):
    if na_rep:
        rendered_df = df.format(na_rep='', precision=2).set_table_styles(pandas_default_style).to_html(header="true", table_id="table")
    else:
        rendered_df = df.format(precision=2).set_table_styles(pandas_default_style).to_html(header="true", table_id="table")

    rendered_df = rendered_df[ : rendered_df.index('<table')+7] + 'style="margin:auto;"' + rendered_df[rendered_df.index('<table')+6:]
    print(f"<div style='width:100%;'>{rendered_df}</div>")

def render_plot(plt, width=None, height=None, multiplier=1.5):
    ...
    plt.close()
    if width and height:
        print(f"<img style='display:block; margin:auto;' height=\"{height}\" width=\"{width}\" src=\"data:image/png;base64, " + pic_hash.decode('utf-8') + "\"/>")
    else:
        fig = plt.gcf()
        width, height = fig.get_size_inches()*fig.dpi
        print(f"<img style='display:block; margin:auto;' height=\"{height*multiplier*0.75}\" width=\"{width*multiplier}\" src=\"data:image/png;base64, " + pic_hash.decode('utf-8') + "\"/>")
...
def calc_diff(l1):
    ...
    for i in range(len(l1)):
        try:
            temp_list.append(round((l1[i+1]-l1[i]), 2))
        except IndexError:
            pass
    temp_list = [l1[0]] + temp_list
    return temp_list
...
def project_at_a_glance(params_dict):
    ...
    params_dict['stock_in_hand'] = round((10*params_dict['revenue'])/(3*100), 2)
...
def project_cost_func(params_dict):
    ...
    project_cost_df.columns = ['Sl No.', 'Items', 'Amount (INR)']\
    
    project_cost_df = project_cost_df.append({
            'Sl No.': project_cost_df.shape[0]+1,
            'Items' : 'Working Capital',
            'Amount (INR)': params_dict['working_capital_loan'] + params_dict['working_capital_loan']*params_dict['marginal_money']
                  }, ignore_index=True)
    
    project_cost_df = project_cost_df.append({
            'Sl No.': '',
            'Items' : 'Total',
            'Amount (INR)': project_cost_df['Amount (INR)'].sum()
                  }, ignore_index=True)
...
def total_yearly_exp(params_dict):
    ...
    total_yearly_exp_df.columns = ['Sl No.', 'Items', 'Amount (INR)']
    total_yearly_exp_df['Amount (INR)'] = total_yearly_exp_df['Amount (INR)']*params_dict['months']
    
    params_dict['total_yearly_exp'] = total_yearly_exp_df['Amount (INR)'].sum()
                     
    total_yearly_exp_df = total_yearly_exp_df.append({
            'Sl No.': '',
            'Items' : 'Total',
            'Amount (INR)': params_dict['total_yearly_exp']
                  }, ignore_index=True)
...
def app_of_fund(project_cost_df, params_dict):
    ...
    app_of_fund_df.rename(columns={'Amount (INR)':'Rate'}, inplace=True)

    app_of_fund_df['Subsidy %'] = params_dict['subsidy']
    app_of_fund_df['No'] = 1
    app_of_fund_df['Amount (INR)'] = app_of_fund_df['Rate']*app_of_fund_df['No']
    ...
    app_of_fund_df = app_of_fund_df.append([{
            ...
            'No': '',
            'Rate': '',
            'Amount (INR)': app_of_fund_df['Amount (INR)'].sum()
                  },
                    {
            'Sl No.': '',
            'Items' : 'Total Subsidy',
            ...
            'No': '',
            'Rate': '',
            'Amount (INR)': sum(app_of_fund_df['Subsidy %']*app_of_fund_df['Amount (INR)'])
                    }]
                , ignore_index=True)
    ...
    app_of_fund_df = app_of_fund_df.append({
            ...
            'No': '',
            'Rate': '',
            'Amount (INR)': app_of_fund_df['Amount (INR)'][app_of_fund_df['Items'].isin(['Total Investment'])].values[0]
                        - app_of_fund_df['Amount (INR)'][app_of_fund_df['Items'].isin(['Total Subsidy'])].values[0]
                  }
                , ignore_index=True)
...
def means_of_finance(params_dict):
    means_of_finance_df = pd.DataFrame(
                                        ...
                                        columns=['Sl No', 'Items', 'Amount (INR)']
                                        )
...
def depreciation_func(params_dict):    
    ...
    for key, value in params_dict['tl'].rate_dict.items():
        if key in params_dict['tl'].dict_info().keys():
...
def cash_flow(params_dict):
    ...
    account_payable = params_dict['total_yearly_exp']*params_dict['credit_purchase_perc']
    account_payable_list = create_row(account_payable, params_dict['multiplier'], params_dict['years'])    
    
    inc_in_curr_liability = [0.00] + calc_diff(account_payable_list)
    ...
    trade_recievables = params_dict['revenue']*params_dict['credit_sell_perc']
    trade_recievables_list = [0.00] + create_row(trade_recievables, params_dict['multiplier'], params_dict['years'])
    inventory = [0.00] + params_dict['closing_stock']
    current_assets = sum_list(trade_recievables_list, inventory)
    
    inc_in_curr_asset = calc_diff(current_assets)
    ...
    params_dict['account_payable_list'] = account_payable_list
    params_dict['inventory'] = inventory
    params_dict['trade_recievables_list'] = trade_recievables_list
    params_dict['closing_balance'] = closing_balance
...
def balance_sheet(params_dict):
    ...
    account_payable_list_bs = [0.00] + params_dict['account_payable_list']
    total_liability = sum_list(capital_list_bs, reserve_surplus, term_loan_list_bs, working_cap_loan_list, account_payable_list_bs)
    
    fixed_assets_bs = [params_dict['fixed_asset_list'][0]] + params_dict['twdv']
    #cash_n_equivalence = subtract_list(total_liability, sum_list(fixed_assets_bs, params_dict['inventory'], params_dict['trade_recievables_list']))
    cash_n_equivalence = params_dict['closing_balance']
    total_asset_list = sum_list(fixed_assets_bs, params_dict['inventory'], params_dict['trade_recievables_list'], cash_n_equivalence)
    ...
    balance_sheet_df = pd.DataFrame(
                                [
                                    ...
                                    account_payable_list_bs,
                                    total_liability,
                                    [],
                                    [],
                                    fixed_assets_bs,
                                    [],
                                    params_dict['inventory'],
                                    params_dict['trade_recievables_list'],
                                    cash_n_equivalence,
                                    total_asset_list
                                ],
...
def project_feasibilty(params_dict):
    ...
    current_ratio = divide_list(sum_list(params_dict['inventory'][1:], params_dict['cash_n_equivalence'][1:], params_dict['trade_recievables_list'][1:]),
                                sum_list(params_dict['working_cap_loan_list'][1:], params_dict['account_payable_list']))
    
    quick_ratio = divide_list(sum_list(params_dict['cash_n_equivalence'][1:], params_dict['trade_recievables_list'][1:])
                              , sum_list(params_dict['working_cap_loan_list'][1:], params_dict['account_payable_list']))
...
def plot_pie(total_yearly_exp_df):
    ...
    plt.pie(x=pie_df['Amount (INR)'],
           labels=pie_df['Items'],
           autopct='%.2f%%',
           explode=[0.1 if pie_df['Amount (INR)'].idxmax()==idx else 0 for idx in pie_df.index],
           shadow=True,
...
def generate_report(kwargs, marginal_money=0.1, years=5, stock_in_hand_perc=2/3, interest=0.11, subsidy=0.00, income_tax=0.00, product='Sales from Fish', quantity=200, rate=2428.54, unit='Kg', no_of_employment=5):
    ...
    params_dict = {'working_capital_loan':kwargs['working_capital_loan'], 'marginal_money':marginal_money, 'years':years,
    ...
    params_dict['credit_purchase_perc'] = 0.016161616
    params_dict['credit_sell_perc'] = 0.01439
    ...
    print("<h1 style='text-align:center'>Project at a Glance</h1>")
    render_dataframe(project_at_a_glance_df.style.hide_columns())
    print("<h1 style='text-align:center'>Project Feasibility Ratio</h1>")
    print(f"<b>Debt Service Coverage Ratio (Average) :</b> {round(sum(params_dict['dscr'])/len(params_dict['dscr']), 2)}<br><br>")
    print(f"<b>Current Ratio (Average) :</b> {round(sum(params_dict['current_ratio'])/len(params_dict['current_ratio']), 2)}<br><br>")
    render_dataframe(project_feasibility_df.style.format(precision=2))
    print("<h1 style='text-align:center'>Line Plot</h1>")
    plot_line(project_feasibility_df=project_feasibility_df)
    print("<h1 style='text-align:center'>Bar Plot</h1>")
    plot_bar(params_dict)
    print("<h1 style='text-align:center'>Pie Chart</h1>")
    plot_pie(total_yearly_exp_df=total_yearly_exp_df)
    print("<h1 style='text-align:center'>Project Cost'</h1>")
    render_dataframe(project_cost_df.style.hide_index())
    print("<h1 style='text-align:center'>Working Capital Computation</h1>")
    render_dataframe(working_capital__computation_df.style.hide_index())
    print("<h1 style='text-align:center'>Annual Sales / Revenue</h1>")
    render_dataframe(df=annual_revenue_df.style.hide_index(), na_rep=True)
    print("<h1 style='text-align:center'>Total Yearly Expense</h1>")
    render_dataframe(total_yearly_exp_df.style.hide_index())
    print("<h1 style='text-align:center'>Application of Fund</h1>")
    render_dataframe(app_of_fund_df.style.hide_index())
    print("<h1 style='text-align:center'>Means of Finance</h1>")
    render_dataframe(means_of_finance_df.style.hide_index())
    print("<h1 style='text-align:center'>Profitability Statement</h1>")
    render_dataframe(df=profitability_statement_df.style, na_rep=True)
    print("<h1 style='text-align:center'>Cash Flow Statement</h1>")
    render_dataframe(cash_flow_statement_df.style, na_rep=True)
    print("<h1 style='text-align:center'>Balance sheet</h1>")
    render_dataframe(balance_sheet_df.style, na_rep=True)
    print("<h1 style='text-align:center'>Repayment of Term Loan</h1>")
    render_dataframe(repayment_schedule_df.style.hide_index())
    print("<h1 style='text-align:center'>Debt Service Coverage Ratio</h1>")
    render_dataframe(dscr_df.style, na_rep=True)
    print(f"<b>Average DSCR :</b> {round(sum(params_dict['dscr'])/len(params_dict['dscr']), 2)}<br><br>")
    print("<h1 style='text-align:center'>Depreciation</h1>")
    render_dataframe(depreciation_df.style, na_rep=True)

if __name__ == "__main__":
    kwargs_list = json.loads(sys.argv[1])
    kwargs = {key:float(value) for ele in kwargs_list for key, value in ele.items()}
    generate_report(kwargs=kwargs, marginal_money=0.25, product='Sales from Soap', rate=53.52, quantity=15000, unit='Units', income_tax=0.3)
